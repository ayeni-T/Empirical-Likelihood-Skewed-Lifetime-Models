...
v5 changes:
  - EJEL removed from all figures
  - AJEL removed from survival figures (Figure 3, Figure 4 S(t0) row,
    Supplementary S3, Supplementary S5)

v6 changes:
  - AJEL replaced by BAJEL (alpha/beta figures and Figure 4 rows)
  - VCJEL added to survival figures (Figure 3, Figure 4 S(t0) row,
    Supplementary S3, Supplementary S5) alongside NA and JEL
  - AJEL/BAJEL remain excluded from survival (structural over-coverage,
    confirmed unaffected by the BAJEL construction itself)
...
if not os.path.exists(CSV_FILE):
    raise FileNotFoundError(
        ...
        "Run: python QTEG_JEL_Arctic_v6.py --merge  first."
    )
...
# ── Method palettes ───────────────────────────────────────────────────
# Alpha/beta: NA, JEL, BAJEL (v6: BAJEL replaces AJEL)
METHODS      = ["NA", "JEL", "BAJEL"]
# Survival: NA, JEL, VCJEL (v6: VCJEL added; AJEL/BAJEL remain excluded
# — structural over-coverage for bounded functionals)
SURV_METHODS = ["NA", "JEL", "VCJEL"]
...
METHOD_STYLES  = {"NA": "-",  "JEL": "--", "BAJEL": "-.", "VCJEL": ":"}
METHOD_MARKERS = {"NA": "s",  "JEL": "o",  "BAJEL": "^",  "VCJEL": "D"}
METHOD_MS      = {"NA": 4.5,  "JEL": 4.5,  "BAJEL": 5.0,  "VCJEL": 4.5}
...
plot_sim_cp(
    cp_cols={"NA": "cp_na_a", "JEL": "cp_jel_a", "BAJEL": "cp_bajel_a"},
    ylabel=r"Coverage probability (%) — $\hat{\alpha}$",
    suptitle=r"Coverage probability for $\alpha$  (nominal = 95%)",
    stem="fig_jel_sim_alpha",
    methods=METHODS,
)

plot_sim_cp(
    cp_cols={"NA": "cp_na_b", "JEL": "cp_jel_b", "BAJEL": "cp_bajel_b"},
    ylabel=r"Coverage probability (%) — $\hat{\beta}$",
    suptitle=r"Coverage probability for $\beta$  (nominal = 95%)",
    stem="fig_jel_sim_beta",
    methods=METHODS,
)

plot_sim_cp(
    cp_cols={"NA": "cp_na_s", "JEL": "cp_jel_s", "VCJEL": "cp_vcjel_s"},
    ylabel=r"Coverage probability (%) — $S(t_0)$",
    suptitle=r"Coverage probability for $S(t_0)$  (nominal = 95%)",
...
def plot_realdata_ci(outfile_stem, nominal=0.95):
    ...
    # Legend: union of every method actually plotted across all rows
    # (alpha/beta use METHODS=[NA,JEL,BAJEL]; S(t0) uses
    # SURV_METHODS=[NA,JEL,VCJEL] -- BAJEL and VCJEL both need their
    # own legend entry since neither list alone covers both rows)
    all_methods_used = []
    for row in ROWS:
        for m in row["methods"]:
            if m not in all_methods_used:
                all_methods_used.append(m)
    leg_handles = _legend_handles(all_methods_used)
    leg_handles += [
    ...
    fig.suptitle(
        f"Real data confidence intervals  (nominal = {int(nominal*100)}%)\n"
...
plot_al(
    al_cols={"NA": "al_na_a", "JEL": "al_jel_a", "BAJEL": "al_bajel_a"},
    ylabel=r"Average length — $\alpha$",
    suptitle=r"Average interval length for $\alpha$  (95%)",
    stem="fig_jel_alpha_al",
    methods=METHODS,
)

plot_al(
    al_cols={"NA": "al_na_b", "JEL": "al_jel_b", "BAJEL": "al_bajel_b"},
    ylabel=r"Average length — $\beta$",
    suptitle=r"Average interval length for $\beta$  (95%)",
    stem="fig_jel_beta_al",
    methods=METHODS,
)

plot_al(
    al_cols={"NA": "al_na_s", "JEL": "al_jel_s", "VCJEL": "al_vcjel_s"},
    ylabel=r"Average length — $S(t_0)$",
    suptitle=r"Average interval length for $S(t_0)$  (95%)",
