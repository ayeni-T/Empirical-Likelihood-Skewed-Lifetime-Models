"""
QTEG_JEL_Arctic.py  (v6 — AJEL replaced by BAJEL; VCJEL added for survival)
===========================================================================
...
Statistical core:
  ...
  - jel_ratio / bajel_ratio                                [Eq. 21,25]
  - _ci_from_ratio       : grid inversion, 1000-point search
  - jel_ci / bajel_ci                                      [Eq. 32-33]
  - survival_pseudovalues_from_pv : reuses stored LOO MLEs [Eq. 39]
  - survival_jel_ci      : plain JEL, grid search over full pseudo-value
                           range [Eq. 41-42]
  - survival_vcjel_ci    : variance-calibrated JEL for S(t0) — shrinks
                           pseudo-values by rho = sigma_delta/sigma_jack
                           before running the JEL construction, correcting
                           the jackknife's known variance-inflation bias
                           for curved/nonlinear functionals
  - na_survival_ci  : logit-transformed Wald CI for S(t0)
                      uses full 2x2 Hessian covariance (cov_ab)

v6 changes vs v5:
  - AJEL replaced by BAJEL (Balanced Adjusted JEL; Zhao, Su & Yang 2020)
    for alpha and beta. BAJEL uses a sign-adaptive pair of artificial
    pseudo-observations rather than AJEL's fixed symmetric pair, giving
    matched coverage with narrower intervals (validated via pilot
    simulation and exact real-data computation: 2.5-4% narrower than
    AJEL across all four real datasets and both parameters).
  - VCJEL added for S(t0): plain JEL over-covers S(t0) (0.2-1.7 points
    at 95% nominal, worsening with skewness) because the delete-1
    jackknife is known to inflate variance for curved, nonlinear
    functionals. VCJEL corrects this directly by rescaling the survival
    pseudo-values toward their mean using rho = sigma_delta/sigma_jack,
    where sigma_delta is the already-available delta-method SD used for
    the NA benchmark. rho is clipped to (0.3, 1.0]: shrink-only, since
    the diagnosed bias only ever inflates jackknife variance, never
    deflates it.
  - AJEL/BAJEL were tested for survival and found NOT to help (their
    over-coverage there is structural, from the two-point augmentation
    itself, not the pseudo-value scale) — AJEL/BAJEL remain excluded
    from survival, as in v5.
  - EJEL remains excluded (v5 decision, advisor feedback).
  - Progress line and result keys updated: cp_ajel_* -> cp_bajel_*,
    a new cp_vcjel_s / al_vcjel_s pair added for survival.
  - ejel_ratio kept in code (used in theory sections) but not called
...
def bajel_ratio(V, theta0, s=1.9):
    """
    BAJEL log-likelihood ratio — Balanced Adjusted JEL
    (Zhao, Su & Yang 2020, Computational Statistics & Data Analysis).
    Replaces AJEL (v6): adds a sign-adaptive pair of artificial
    pseudo-observations rather than a fixed symmetric pair, matching
    AJEL's coverage with a narrower confidence region. Tuning constant
    s=1.9 follows the source paper. Used for alpha and beta only.
    """
    W = V - theta0
    n = len(W)
    Wbar = np.mean(W)
    s_jack = np.std(W, ddof=1)
    u_star = np.sign(Wbar) if Wbar != 0 else 0.0
    c_u = u_star * s_jack
    Wn1 = -s * c_u
    Wn2 = 2.0 * Wbar + s * c_u
    W_aug = np.concatenate([W, [Wn1, Wn2]])
    try:
        lam = _solve_lambda(W_aug, 0.0)
...
def bajel_ci(V, nominal, theta_hat):
    return _ci_from_ratio(bajel_ratio, V, nominal, theta_hat)
...
def na_survival_ci(ah, bh, se_a, se_b, t0, nominal, cov_ab=0.0):
    ...
    Uses full 2x2 covariance matrix. Variance computed via
    delta_var_psi (shared with VCJEL's calibration step, see below).
    """
    var_psi = delta_var_psi(ah, bh, t0, se_a, se_b, cov_ab)
...
def survival_jel_ci(V_surv, nominal):
    """
    Plain JEL CI for S(t0). Grid searches the full pseudo-value convex
    hull rather than restricting to (0,1), which corrects interval
    length inflation caused by pseudo-values extending outside (0,1).
    Results clipped to [0,1] since S(t0) is a probability.
    AJEL/BAJEL are not included for survival: diagnostics confirmed
    that their over-coverage for S(t0) is structural (caused by the
    two-point augmentation itself, not the pseudo-value scale) and
    cannot be corrected by tuning. NA, JEL, and VCJEL are reported.
    """
...
def delta_var_psi(ah, bh, t0, se_a, se_b, cov_ab, h=1e-5):
    """Delta-method variance of psi_hat = S(t0; alpha, beta), used both
    for the NA benchmark (na_survival_ci) and to calibrate VCJEL."""
    dS_da = (qteg_sf(t0, ah+h, bh) - qteg_sf(t0, ah-h, bh)) / (2*h)
    dS_db = (qteg_sf(t0, ah, bh+h) - qteg_sf(t0, ah, bh-h)) / (2*h)
    return (dS_da*se_a)**2 + (dS_db*se_b)**2 + 2.0*dS_da*dS_db*cov_ab


def survival_vcjel_ci(V_surv, nominal, var_delta, n):
    """
    VCJEL — Variance-Calibrated JEL for S(t0) (v6, new).

    Plain JEL over-covers S(t0) by 0.2-1.7 points at 95% nominal,
    worsening with skewness, because the delete-1 jackknife is known
    to inflate variance for curved, nonlinear functionals such as
    S(t0; alpha, beta). VCJEL corrects this directly: the survival
    pseudo-values are rescaled toward their mean by
        rho = sigma_delta / sigma_jack,
    where sigma_delta is the delta-method SD already used for the NA
    benchmark (delta_var_psi) and sigma_jack is the empirical SD of
    the raw pseudo-values. rho is clipped to (0.3, 1.0]: this is a
    shrink-only correction, since the diagnosed mechanism only ever
    inflates jackknife variance, never deflates it, and clipping at
    0.3 guards against instability from a poorly estimated sigma_jack
    in rare degenerate samples.

    Validated (Monte Carlo pilot + exact real-data computation): rho
    is close to 1 (no correction) at larger n and in the two largest
    real datasets (DS2, DS3), and meaningfully below 1 (~0.955) in the
    two smaller real datasets (DS1, DS4) -- consistent with the
    diagnosed finite-sample mechanism vanishing asymptotically.
    """
    V_surv = np.asarray(V_surv, float)
    sd_jack = np.std(V_surv, ddof=1)
    if sd_jack <= 0 or not np.isfinite(var_delta):
        rho = 1.0
    else:
        sd_delta = np.sqrt(max(var_delta, 1e-12)) * np.sqrt(n)
        rho = float(np.clip(sd_delta / sd_jack, 0.3, 1.0))

    Vbar = np.mean(V_surv)
    V_cal = Vbar + rho * (V_surv - Vbar)
    lo, hi = survival_jel_ci(V_cal, nominal)
    return lo, hi, rho
...
def run_block(block_id, n_sim=N_SIM_DEFAULT):
    ...
    # EJEL removed (v5); AJEL replaced by BAJEL for alpha/beta (v6);
    # VCJEL added for survival (v6)
    counts = {k: 0 for k in [
        'n_conv',
        'cp_na_a',   'al_na_a',
        'cp_jel_a',  'al_jel_a',
        'cp_bajel_a','al_bajel_a',
        'cp_na_b',   'al_na_b',
        'cp_jel_b',  'al_jel_b',
        'cp_bajel_b','al_bajel_b',
        'cp_na_s',   'al_na_s',
        'cp_jel_s',  'al_jel_s',
        'cp_vcjel_s','al_vcjel_s',
        'sum_rho_s',
    ]}
    ...
    for rep in range(resume_from, n_sim):
        ...
        # ── JEL and BAJEL for alpha ───────────────────────────────────
        for fn, key in [(jel_ci,'jel_a'), (bajel_ci,'bajel_a')]:
        ...
        # ── JEL and BAJEL for beta ────────────────────────────────────
        for fn, key in [(jel_ci,'jel_b'), (bajel_ci,'bajel_b')]:
        ...
        # ── Survival: NA, JEL, and VCJEL (AJEL/BAJEL excluded — ───────
        # ── structural over-coverage for bounded functionals) ─────────
        ...
        if np.isfinite(se_a) and np.isfinite(se_b):
            var_delta = delta_var_psi(ah, bh, t0, se_a, se_b, cov_ab)
            vlo, vhi, rho = survival_vcjel_ci(sv['V'], nominal, var_delta, sv['n'])
            if np.isfinite(vlo) and np.isfinite(vhi):
                if vlo > vhi: vlo, vhi = vhi, vlo
                if vlo <= psi_true <= vhi: counts['cp_vcjel_s'] += 1
                counts['al_vcjel_s'] += vhi - vlo
            counts['sum_rho_s'] += rho
        ...
        if (rep + 1) % CHECKPOINT_EVERY == 0:
            ...
            print(f"  Rep {rep+1:>5}/{n_sim}  "
                  ...
                  f"CP(S)VCJEL={100*counts['cp_vcjel_s']/nc:>5.1f}%  "
                  f"CP(α)BAJEL={100*counts['cp_bajel_a']/nc:>5.1f}%")
...
def run_real_data(nominal=0.95):
    ...
    for ds_name, y in DATASETS.items():
        ...
        # CIs for alpha — NA, JEL, BAJEL (EJEL removed v5; AJEL->BAJEL v6)
        na_a    = na_ci(ah, se_a, nominal)
        jel_a   = jel_ci(pva, nominal, ah)
        bajel_a = bajel_ci(pva, nominal, ah)

        # CIs for beta — NA, JEL, BAJEL
        na_b    = na_ci(bh, se_b, nominal)
        jel_b   = jel_ci(pvb, nominal, bh)
        bajel_b = bajel_ci(pvb, nominal, bh)

        # Survival CIs — NA, JEL, VCJEL (AJEL/BAJEL excluded: structural
        # over-coverage for bounded functionals)
        ...
        jel_s  = (jlo, jhi)
        var_delta = delta_var_psi(ah, bh, t0, se_a, se_b, cov_ab)
        vlo, vhi, rho = survival_vcjel_ci(sv_pv['V'], nominal, var_delta, sv_pv['n'])
        vcjel_s = (vlo, vhi)

        sv = dict(psi_hat=sv_pv['psi_hat'],
                  psi_jack=sv_pv['psi_jack'],
                  na=na_s, jel=jel_s, vcjel=vcjel_s, rho=rho)
        ...
        print(f"    BAJEL: {fmt_ci(bajel_a)}")

        print(f"\n  CIs for beta ({int(nominal*100)}%):")
        print(f"    NA:    {fmt_ci(na_b)}")
        print(f"    JEL:   {fmt_ci(jel_b)}")
        print(f"    BAJEL: {fmt_ci(bajel_b)}")

        print(f"\n  Survival S({t0}) = {sv['psi_hat']:.4f}")
        print(f"    NA    CI: {fmt_ci(sv['na'])}")
        print(f"    JEL   CI: {fmt_ci(sv['jel'])}")
        print(f"    VCJEL CI: {fmt_ci(sv['vcjel'])}  (rho={rho:.4f})")

        all_results[ds_name] = dict(
            fit=fit, pv=pv,
            ci_alpha=dict(na=na_a, jel=jel_a, bajel=bajel_a),
            ci_beta=dict(na=na_b, jel=jel_b, bajel=bajel_b),
            survival=sv, t0=t0
        )
...
def merge_results():
    ...
    print(f"\n{'Sc':<4} {'Nom':>5} {'n':>4}  "
          f"{'CP_α NA':>8} {'CP_α JEL':>9} {'CP_α BAJEL':>11}  "
          f"{'AL_α JEL':>9}  {'CP_S NA':>8} {'CP_S JEL':>9} {'CP_S VCJEL':>11}")
    print("-"*100)
    for r in sorted(results,
                    key=lambda x: (x['scenario'], x['nominal'], x['n'])):
        print(f"Sc.{r['scenario']} {r['nominal']:>5.0%} {r['n']:>4}  "
              f"{r['cp_na_a']:>8.2f} {r['cp_jel_a']:>9.2f} "
              f"{r['cp_bajel_a']:>11.2f}  "
              f"{r['al_jel_a']:>9.4f}  "
              f"{r.get('cp_na_s', float('nan')):>8.2f} "
              f"{r['cp_jel_s']:>9.2f} "
              f"{r.get('cp_vcjel_s', float('nan')):>11.2f}")

# ── Entry point ───────────────────────────────────────────────────────

if __name__ == '__main__':
    parser = argparse.ArgumentParser(
        description='QTEG JEL simulation — Arctic HPC (v6)')
    ...
    if args.realdata:
        ...
        print("Usage:")
        print("  Test:      python QTEG_JEL_Arctic_v6.py --test")
        print("  Block:     python QTEG_JEL_Arctic_v6.py --block 0")
        print("  Merge:     python QTEG_JEL_Arctic_v6.py --merge")
        print("  Real data: python QTEG_JEL_Arctic_v6.py --realdata")
        print("  Via SLURM: sbatch qteg_jel_array_v6.sh")
